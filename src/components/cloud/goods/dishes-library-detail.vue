<template>
  <div class="dishes-library-detail">
    <Steps :current="step">
    ...
    <div class="step3 step-section" v-show="step === 2">
    ...
    <div class="action-section">
      ...
      <Button v-show="step > 0 && step <= 2" @click="step--">上一步</Button>
      <Button class="next-btn" v-show="step >= 0 && step < 2" @click="next">下一步</Button>
      <Button class="ok-btn" v-show="step === 2" @click="ok">完成</Button>
    </div>
  </div>
</template>
...
/** 接口地址 */
const SvcUrl = {
  AddDishes: API.BRAND + '/addBrandDishes',
  ModifyDishes: API.BRAND + '/modifyBrandDishes',
  ModifyDishesSide: API.BRAND + '/modifyBrandDishesSide',
  Upload: API.RESOURCE,
  AddDishSpc: API.BRAND + '/addBrandDishSpc',
  DeleteDishSpc: API.BRAND + '/deleteBrandDishSpc',
  ToggleDishSpcDefault: API.BRAND + '/modifyBrandDefaultSpc'
};
  ...
  methods: {
    ...
    apiModifyDishesSide(item) {
      let param = {
        ...
        sideDishesInfoList: item.sideDishesInfoList
      };
                ...
                click: () => {
                  ...
                  if (this.editType === Tool.EditTypeEnum.Create) {
                    ...
                    let item = {
                      ...
                      brandDishesVersion: this.value.brandDishesVersion,
                      id: spec.id
                    };
                    ...
                    this.apiUpdateDishSpec(item)
                      .then(data => {
                        this.value.brandDishesVersion = data.brandDishesVersion;
                        done();
                      }, err => {
      ...
      this.$refs.specForm.validate(valid => {
        if (valid) {
          ...
          if (this.editType === Tool.EditTypeEnum.Create) {
            ...
            item.brandDishesVersion = this.value.brandDishesVersion;
